...
from UnoPattern_class import *
import csv

CSV_WRITE_VEC_NUM = 10000

def create_scan_template(uno_pattern_file:str):
    file = uno_pattern_file.split('/')[-1]
    path = uno_pattern_file.replace(f"{file}", '')
    
    print(">>> " + file)
    uno_pattern = UnoPattern(uno_pattern_file)

    uno_pattern.get_pin_list_pins()
    print("PinList number:", uno_pattern.PinList_number)
    
    uno_pattern.get_subset_pin_list_pins()
    print("SubsetPins number:", uno_pattern.SubsetPins_number)

    output_path = str(path).replace(f"{INPUT_FOLDER}", f"{OUTPUT_FOLDER}") + '/'
    output_file = str(file).replace('.uno', '.csv')
    ...
    df = uno_pattern.create_pad_alias_dataframe()
    print("pattern depth:", uno_pattern.uno_depth)        
    ...
    if re.search(r"_SSN", str(file), re.IGNORECASE):
        for pad in df.columns:
            ...
            output_file = str(file).replace('.uno', '') + '_unique.csv'
    ...
    output_file = str(file).replace('.uno', '') + '_template.txt'
    write_type_to_csv = ['']
    write_logic_to_csv = ['']
    ...
    for vec in uno_pattern.alias:
        if str(uno_pattern.alias[vec]).find('H') != -1:
...
def main():
    ...
    create_scan_template('./input/uno/ATPG_OCC_PA_comp_chain_EDT_0P85V_20250912_v1.uno')
    
    # paths, files = Search_files(INPUT_FOLDER, ['.uno'])
    # for path, file in zip(paths, files):
    #     create_scan_template(path + '/' + file)
