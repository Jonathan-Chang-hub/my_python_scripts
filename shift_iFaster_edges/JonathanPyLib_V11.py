...
class InputFiles:
    file_path  = []
    source_files = []
    def __init__(self, folder_path, FileExt):
        self.folder_path = folder_path
        self.FileExt = FileExt
    def SearchInputFiles(self):
        for root, dirs, files in os.walk(self.folder_path):
            for file in files:
                if file.endswith(self.FileExt):
                    # excel_files.append(os.path.join(root, file))
                    self.file_path.append(root)
                    self.source_files.append(file)
    def GlobWantFileExt(self):
        Dmx_All_log_Array = []
        for All_log_file in glob.glob('*'+self.FileExt):
            print('The current folder All datalog file is = %s'%All_log_file)
            Dmx_All_log_Array.append(All_log_file)


def Search_files(folder_path, FileExt):
    file_path  = []
    source_files = []
    # 遍历指定文件夹及其子文件夹
...
class ReadWriteFiles:
    ReadContent = ""
    encode = 'utf-8'
    def ReadFileLines(self, FileOpenPath):
        with open(FileOpenPath, 'r', encoding=self.encode) as fr:
            self.ReadContent = fr.readlines()
        fr.close()
    def WriteFileLines(self, FileWritePath, WritedContentLine:list):
        with open(FileWritePath, 'w', encoding=self.encode) as fw:
            for k in range(len(WritedContentLine)):
                fw.write(WritedContentLine[k])
        fw.close()
    def ReadFile(self, FileOpenPath):
        with open(FileOpenPath, 'r', encoding=self.encode) as fr:
            self.ReadContent = fr.read()
        fr.close()
    def WriteFile(self, FileWritePath, WritedContent):
        with open(FileWritePath, 'w', encoding=self.encode) as fw:
            fw.write(WritedContent)
    def AppendFile(self, FileWritePath, WritedContent):
        with open(FileWritePath, 'a', encoding=self.encode) as fa:
            fa.write(WritedContent)
        fa.close()


def ReadFileLines(FileOpenPath, encode):
    with open(FileOpenPath, 'r', encoding=encode) as fr:
        FileContent = fr.readlines()
    fr.close()
    return FileContent


def WriteFileLines(FileOpenPath, encode, FileContent):
    with open(FileOpenPath, 'w', encoding=encode) as fw:
            for k in range(len(FileContent)):
                fw.write(FileContent[k])
    fw.close()


def ReadFile(FileOpenPath, encode):
    with open(FileOpenPath, 'r', encoding=encode) as fr:
        FileContent = fr.read()
    fr.close()
    return FileContent
...
def WriteFile(FileOpenPath, encode, FileContent):
    with open(FileOpenPath, 'w', encoding=encode) as fw:
...
def unzip_gz(file_name, outputfolder, UnzipAndWrite:bool ,otheroutput:bool):
